...
def make_grid(init_centers: list,chipcolumns: int,chiprows: int,removed: list) -> list:
    ...
    if chiprows % 2:
        range_y = range(-int(chiprows/2),int(chiprows/2)+1)
    else:
    ...
    try:
        ...
        print("There is a problem with the chip-coordinates you want to delete")
        sys.exit()
...
"""
The points need to be in the exact corner of the chips. The chips in the x-direction are the most important, you should measure
the lower left corner of the chip in the middle, if there is an even number of chips you choose the chip with the lowest y-value
The chips in the x-direcdtion should also be measured in the lower left corner, but it is not too important that they are chosen

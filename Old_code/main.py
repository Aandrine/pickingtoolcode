...
from globals import callibration_lengt, callibragtion_tolerancea, callibragtion_tolerance
...
def detect_squares(square_image, min_square_size, max_square_size, indices):
    centers_squares = []
    center = None  # center wird zu Beginn auf None gesetzt
    num_center = 0
    angle = 0
    detect_squares_img = square_image.copy()  # Kopiere das Bild für die Verarbeitung

    # Logik zum Erkennen der Quadrate
    for index in indices:
        # Hier erfolgt die Erkennung des Quadrats (dies ist ein Platzhalter, du musst es an deinen Code anpassen)
        detected_square = square_image[index]  # Beispiel für das Extrahieren eines Quadrats

        if detected_square is not None:
            # Berechne den Mittelpunkt des Quadrats (dies ist eine Dummy-Berechnung, du musst es entsprechend anpassen)
            center = (detected_square[0] + detected_square[1]) / 2  # Beispielhafte Berechnung des Mittelpunkts
            centers_squares.append(center)
            num_center += 1
        else:
            # Falls kein Quadrat gefunden wurde, setze center auf None
            center = None

    # Wenn center nie gesetzt wurde (also kein Quadrat gefunden wurde), setze center auf einen Standardwert
    if center is None:
        center = (0, 0)  # Standardwert, falls kein Quadrat gefunden wurde
...
# Funktion to get the wafer map
    ...
    if file:
        ...
        try:
            ...
            update_scrollable_frame('Robot connection failed!', color="red")
...
#Funktion to get the Vision Data
    ...
    if chip_quality_array == []:
        update_scrollable_frame('No wafer map loaded: Import a wafer map first!', color="red")
        return
    else:
    ...
    if len(vision_data) != 36:
        update_scrollable_frame(f'Number of chips incorrect: Current number {len(vision_data)}', color="red")
    else:
